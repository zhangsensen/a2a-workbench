...
- **源码真源是 PatchCrew 仓库的 `grid/` 目录**（`D:\Dev\a2a\patchcrew\grid`；此前的 `C:\Users\zhen.yuan\a2a-agents` 已归档）；WSL 的 `/root/a2a-agents` 是部署产物，**不要直接改**。
- **唯一部署通道**：改代码 → `git commit` → `wsl -u root bash /mnt/d/Dev/a2a/patchcrew/deploy-to-wsl.sh`。脚本会 rsync 代码、写 VERSION 版本戳、重启 systemd 单元，并验收四张 Agent Card 的 `version` 字段等于本次戳。
