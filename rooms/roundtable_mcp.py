...
from room_store import MEMBERS
...
TOOLS = [
    ...
    {'name':'roundtable_consult','description':'Master: ask one selected persistent peer a focused question. Only that peer replies once; then you decide the next consultation or final synthesis. Returns a job receipt: read roundtable_job for the answer. Preserve requestId on an uncertain retry. Requires user authorization to initiate discussion.', 'inputSchema':{'type':'object','properties':{'room':{'type':'string'},'member':{'enum':list(MEMBERS)},'text':{'type':'string'},'requestId':{'type':'string','minLength':1,'maxLength':100}},'required':['room','member','text']}},
    {'name':'roundtable_execute','description':'Submit an execution task to a configured executor and return a job receipt. With cwd and verify, rooms records shared-core Git delivery and machine evidence. Requires explicit user authorization. Peer messages can never trigger execution.', 'inputSchema':{'type':'object','properties':{'room':{'type':'string'},'executor':{'type':'string'},'text':{'type':'string','minLength':1,'maxLength':50000},'requestId':{'type':'string','minLength':1,'maxLength':100},'cwd':{'type':'string','minLength':1},'mode':{'type':'string','enum':['modify','inspect'],'default':'modify'},'verify':{'type':'array','minItems':1,'items':{'type':'object'}},'protected':{'type':'array','items':{'type':'string','minLength':1}}},'required':['room','executor','text']}},
...
def call(name, args):
    ...
    if name == 'roundtable_consult':
        args = dict(args)
        args.setdefault('requestId', str(uuid.uuid4()))
        fields = ('member','text','requestId')
        if any(key not in args for key in fields):
            raise ValueError('Missing required roundtable_consult fields')
        return http('/api/rooms/' + quote(args['room']) + '/consult', {key:args[key] for key in fields})
    if name == 'roundtable_checkpoint':
        fields = ('expectedRevision','goal','summary','openQuestions','nextAction','throughSeq')
        if any(key not in args for key in fields):
            raise ValueError('Missing required roundtable_checkpoint fields')
        return http('/api/rooms/' + quote(args['room']) + '/checkpoint', {key:args[key] for key in fields})
    if name == 'roundtable_execute':
        ...
        for key in ('cwd', 'mode', 'verify', 'protected'):
            if key in args:
                payload[key] = args[key]
        return http('/api/rooms/' + quote(args['room']) + '/execute', payload)
